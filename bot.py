...
DEFAULT_PRODUCTS = {
    ...
    "kfc_600_799": {"name": "KFC 800 pts", "price": 3.25, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    "kfc_800_999": {"name": "KFC 1000 pts", "price": 3.75, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    "kfc_1000_1299": {"name": "KFC 1300 pts", "price": 4.50, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    "kfc_1300_1599": {"name": "KFC 1600 pts", "price": 5.50, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    "kfc_1600_1799": {"name": "KFC 1800 pts", "price": 6.50, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    "kfc_1800_2099": {"name": "KFC 2000 pts", "price": 7.0, "category": "fastfood", "subcategory": "kfc", "type": "manual", "active": True},
    ...
    "tech_steam": {"name": "Tech Steam", "price": 20.0, "category": "tech", "subcategory": "steam_tech", "type": "manual", "active": True},
...
def load_data():
    ...
    for migrated_id in ["refund_eneba", "refund_uber"]:
        if migrated_id in data["products"]:
            ...
            data["products"][migrated_id]["type"] = DEFAULT_PRODUCTS[migrated_id]["type"]
            data["products"][migrated_id]["type"] = DEFAULT_PRODUCTS[migrated_id]["type"]
    for migrated_id in ["tech_uber_50", "tech_mcdo_50", "tech_steam"]:
        if migrated_id in data["products"]:
            data["products"][migrated_id]["name"] = DEFAULT_PRODUCTS[migrated_id]["name"]
            data["products"][migrated_id]["price"] = DEFAULT_PRODUCTS[migrated_id]["price"]
            data["products"][migrated_id]["active"] = True
...
def tech_menu():
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("🎮 Tech Steam", callback_data="techsubcat:steam_tech")],
            [InlineKeyboardButton("⬅️ Retour", callback_data="menu:shop")],
        ]
    )
...
async def show_ubereats_intro(query):
    ...
    text = (
        ...
        "2️⃣ Le <b>sous-total</b> de ta commande doit etre compris entre <b>20€ et 23€</b>.\n"
        "⚠️ On parle bien du <b>sous-total</b>, pas du total avec taxes et frais.\n\n"
        "3️⃣ Nous calculons ensuite le montant a regler.\n"
        ...
        "4️⃣ Effectue le paiement via crypto, PayPal, Apple Pay ou Google Pay.\n\n"
        "5️⃣ Une fois le paiement valide, je tente de passer la commande.\n\n"
        "6️⃣ Si je n'arrive pas a passer la commande dans un delai de 30 minutes, tu recois un remboursement complet.\n\n"
...
async def show_fastfood_subcategory(query, subcategory):
    ...
    if subcategory == "mcdo":
        rows = [
            [InlineKeyboardButton("🌐 Ouvrir le site", url=MCDO_SITE_LINK)],
            [InlineKeyboardButton("⬅️ Retour", callback_data="cat:fastfood")],
        ]
        text = (
            "🍔 McDo\n\n"
            "✨ Les commandes McDo passent maintenant directement sur notre site.\n"
            "Tu peux commander plus simplement et payer en PayPal ou en crypto.\n\n"
            f"🌐 Site web : {MCDO_SITE_LINK}\n\n"
            "Merci pour ta confiance et bonne commande !"
        )
        await edit_or_reply(
            query.message,
            text,
...
async def show_product(query, product_id):
    ...
    site_redirect_products = {
        "snapchat": ("👻 Snapchat+", "Les commandes Snapchat+ passent maintenant directement sur notre site."),
        "snapchat_ss06": ("👻 SS.006", "Les commandes SS.006 passent maintenant directement sur notre site."),
        "refund_uber": ("💸 Refund Uber", "Les commandes Refund Uber passent maintenant directement sur notre site."),
        "refund_eneba": ("💸 Refund Eneba", "Les commandes Refund Eneba passent maintenant directement sur notre site."),
        "amazon_prime": ("📦 Amazon Prime", "Les commandes Amazon Prime passent maintenant directement sur notre site."),
        "prime_video_lifetime": ("📺 Prime Video", "Les commandes Prime Video passent maintenant directement sur notre site."),
    }
    if product_id in site_redirect_products:
        title, intro = site_redirect_products[product_id]
        text = (
            f"{title}\n\n"
            f"✨ {intro}\n"
            "Tu peux commander plus simplement et payer en PayPal ou en crypto.\n\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_state.get("action") == "ubereats_address":
        ...
        save_data()
        await update.message.reply_text(
            "💶 Envoie maintenant le sous-total de la commande Uber Eats.\n\nLe sous-total doit etre compris entre 20€ et 23€.\nExemple : 21.90"
        )
        return
    if user_state.get("action") == "ubereats_total":
        ...
        if amount < 20 or amount > 23:
            await update.message.reply_text("❌ Montant refuse.\n\nReessaie avec un sous-total compris entre 20€ et 23€.")
            return
