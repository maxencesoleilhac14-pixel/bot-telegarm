...
def load_data():
    ...
    for product_id, product in data["products"].items():
        if product_id.startswith("mcdo_"):
            ...
            product["category"] = "fastfood"
            product["type"] = "manual"
        elif product_id.startswith("giftcard_custom_") or product_id in {"card_zalando", "card_boulanger", "card_carrefour", "card_conforama"}:
    ...
    for migrated_id in ["netflix", "crunchyroll", "chatgpt", "chatgpt_1y", "disney_plus", "basic_fit", "basic_fit_1y", "gemini_nitro_ai", "snapchat", "snapchat_ss06", "card_zalando", "card_boulanger", "card_carrefour", "card_conforama", "lifetime_steam", "rockstar_activation_code", "fivem_ready_lifetime", "minecraft_lifetime"]:
        if migrated_id in data["products"]:
            data["products"][migrated_id]["name"] = DEFAULT_PRODUCTS[migrated_id]["name"]
            data["products"][migrated_id]["price"] = DEFAULT_PRODUCTS[migrated_id]["price"]
