...
DEFAULT_PRODUCTS = {
    ...
    "crunchyroll": {"name": "Crunchyroll a vie", "price": 6.0, "category": "subscriptions", "subcategory": "crunchyroll", "type": "manual", "active": True},
    ...
    "disney_plus": {"name": "Disney+ a vie", "price": 9.99, "category": "subscriptions", "subcategory": "disney_plus", "type": "manual", "active": True},
...
def load_data():
    ...
    for product_id, product in data["products"].items():
        if product_id.startswith("mcdo_"):
            ...
            product["category"] = "fastfood"
            product["type"] = "manual"
        elif product_id.startswith("giftcard_custom_") or product_id in {"card_zalando", "card_carrefour", "card_conforama"}:
            product["category"] = "giftcards"
            product["subcategory"] = product.get("subcategory") or DEFAULT_PRODUCTS.get(product_id, {}).get("subcategory")
            product["type"] = "manual"
        elif product_id in {"spotify_premium", "netflix", "crunchyroll", "chatgpt", "chatgpt_1y", "disney_plus", "basic_fit", "basic_fit_1y"}:
            product["category"] = "subscriptions"
            product["subcategory"] = DEFAULT_PRODUCTS.get(product_id, {}).get("subcategory")
            product["type"] = "manual"
        elif product_id in {"snapchat", "snapchat_ss06"}:
            product["category"] = "tech"
            product["subcategory"] = "snapchat"
            product["type"] = "manual"
    ...
    for migrated_id in ["netflix", "crunchyroll", "chatgpt", "chatgpt_1y", "disney_plus", "basic_fit", "basic_fit_1y", "snapchat", "snapchat_ss06", "card_zalando", "card_carrefour", "card_conforama"]:
        if migrated_id in data["products"]:
            data["products"][migrated_id]["name"] = DEFAULT_PRODUCTS[migrated_id]["name"]
            data["products"][migrated_id]["price"] = DEFAULT_PRODUCTS[migrated_id]["price"]
...
def admin_payment_methods_menu():
    ...
    for method, config in PAYMENT_METHODS.items():
        ...
        rows.append([InlineKeyboardButton(f"{status} {config['label']} -> {action}", callback_data=f"admin:paytoggle:{method}")])
    rows.append([InlineKeyboardButton("⬅️ Retour", callback_data="admin:home")])
    return InlineKeyboardMarkup(rows)
...
def admin_loyalty_categories_menu(selected):
    ...
    for category in ["fastfood", "subscriptions", "tech", "giftcards", "boosts", "refunds", "ubereats", "osint"]:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pay:method:"):
        method = data.split(":")[2]
        if method != "balance" and not payment_method_enabled(method):
            total = 0.0
    ...
    if data.startswith("admin:paytoggle:"):
        ...
        await edit_or_reply(
            query.message,
            f"💳 {PAYMENT_METHODS[method]['label']} est maintenant {state_text}.",
            reply_markup=admin_payment_methods_menu(),
        )
        return
